...
as $$
  with ranked as (
    select
      ...
      ts_rank_cd(chunks.search_vector, websearch_to_tsquery('english', query_text)) as text_rank
  ...
  select
    ...
    ranked.content,
    (
      ranked.vector_similarity * 0.72
      + (ranked.text_rank / (ranked.text_rank + 1)) * 0.28
...
revoke all on function public.match_course_chunks_hybrid(
  extensions.vector,
  text,
  uuid,
  integer,
...
grant execute on function public.match_course_chunks_hybrid(
  extensions.vector,
  text,
  uuid,
  integer,
